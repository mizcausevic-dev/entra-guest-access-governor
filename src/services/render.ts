...
function layout(title: string, active: string, body: string) {
  const nav = [
    { href: "/", label: "Overview" },
    { href: "/guest-lane", label: "Guest Lane" },
    { href: "/access-gaps", label: "Access Gaps" },
...
<html lang="en">
  ...
  <body>
    ...
    <div class="wrap">
      <div class="topbar">
        <div class="left">Kinetic Gain · Entra Guest Access Governor</div>
        <div class="right">
          <div>synthetic guest exports · sponsor packets</div>
          <div>entra b2b · access reviews · cross-tenant trust</div>
        </div>
      </div>
      <div class="herorow">
        <section class="hero">
          <div class="chiprow">
            <span class="meta-chip">Wave 15 · Identity and Endpoint Expansion</span>
            <span class="meta-chip">Microsoft / Entra / B2B guest governance proof</span>
            <span class="meta-chip">Synthetic guest snapshots + review exports</span>
          </div>
          <h1>Guest invitations, sponsor drift, and cross-tenant trust that stay operator-readable.</h1>
          <p>This control plane turns Entra guest-access snapshots into one identity-governance surface: invitation sprawl, missing access reviews, stale sponsors, inactive guests, cross-tenant trust gaps, and the remediation packets needed before external collaboration drifts.</p>
          <div class="navrow">
        ...
        <aside class="side">
          <div class="bluf">
            ...
            <p><strong>Entra B2B guest access, access review, and sponsor-governance operations for IAM and security teams.</strong><br />Audit-safe visibility into guest lifecycle, review posture, sponsor ownership, and cross-tenant trust without exposing tenant credentials.</p>
          </div>
          <div class="corr">
            <div class="lbl">Proof Layer</div>
            <p><strong>Offline posture analyzer + CLI + dashboard surface.</strong><br />This repo reads guest-access exports and turns them into owner, control-gap, and remediation packets.</p>
          </div>
          <div class="corr">
            <div class="lbl">Why it matters</div>
            <p>Recruiters looking for <strong>Azure / Entra / guest access / access reviews / identity governance</strong> should see real admin work: B2B lifecycle, sponsor evidence, and trust-boundary cleanup proof.</p>
          </div>
        </aside>
      </div>
      ${body}
      <div class="footer">
        <div>entra-guest-access-governor · synthetic sample data only</div>
        <div>routes: / · /guest-lane · /access-gaps · /review-posture · /verification · /docs</div>
      </div>
    </div>
  </body>
</html>`;
...
export function renderOverview() {
  ...
  return layout(
    ...
    `<section class="section">
        <div class="sh"><h2>Operator Snapshot</h2><div class="note">guest lifecycle · reviews · sponsors · trust posture</div></div>
        <div class="kpis">
          <div class="kpi cyan"><div class="v">${metrics.bundles}</div><div class="lbl">guest snapshots</div><div class="h">Synthetic Entra guest governance bundles across B2B collaboration scopes.</div></div>
          <div class="kpi green"><div class="v">${metrics.currentBundles}</div><div class="lbl">current bundles</div><div class="h">Snapshots fresh enough to trust for renewal and review decisions.</div></div>
          <div class="kpi plum"><div class="v">${metrics.gaps}</div><div class="lbl">gaps</div><div class="h">Observed control deviations across invitations, reviews, sponsors, trust, inactivity, and telemetry.</div></div>
          <div class="kpi red"><div class="v">${metrics.blockingGaps}</div><div class="lbl">blocking gaps</div><div class="h">Changes actively weakening the expected guest-governance posture.</div></div>
          <div class="kpi amber"><div class="v">${metrics.guestRisks}</div><div class="lbl">guest risks</div><div class="h">Invitation sprawl, inactive accounts, or orphaned access still need cleanup.</div></div>
          <div class="kpi red"><div class="v">${metrics.reviewRisks}</div><div class="lbl">review risks</div><div class="h">Access review, sponsor, or cross-tenant trust posture still needs repair.</div></div>
        </div>
      </section>
      <section class="section">
        <div class="sh"><h2>Why operators care</h2><div class="note">entra proof · recruiter signal</div></div>
        <div class="stack">
          <div class="src"><div class="src-name">guardrails first</div><div class="src-tit">Repair guest posture before renewing access</div><p>${metrics.recommendation}</p></div>
          <div class="src"><div class="src-name">control evidence</div><div class="src-tit">Turn guest exports into operator proof</div><p>Every lane stays tied to owner, control family, resource path, and the next concrete remediation move.</p></div>
          <div class="src"><div class="src-name">recruiter signal</div><div class="src-tit">Show real Entra B2B depth</div><p>This is real guest lifecycle and access-review proof, not generic cloud copy.</p></div>
        </div>
      </section>`
  );
}

export function renderGuestLane() {
  return layout(
    "Entra Guest Access Governor — Guest Lane",
    "/guest-lane",
    `<section class="section">
        <div class="sh"><h2>Guest Lane</h2><div class="note">owner · focus · next action</div></div>
        <table class="ttbl">
          <thead>
            <tr>
              <th>Guest lane</th>
...
export function renderAccessGaps() {
  return layout(
    "Entra Guest Access Governor — Access Gaps",
    "/access-gaps",
    `<section class="section">
        <div class="sh"><h2>Access Gaps</h2><div class="note">severity · owner · resource</div></div>
        <table class="ttbl">
          <thead>
            <tr>
              <th>Risk</th>
...
export function renderReviewPosture() {
  return layout(
    "Entra Guest Access Governor — Review Posture",
    "/review-posture",
    `<section class="section">
        <div class="sh"><h2>Review Posture</h2><div class="note">packet readiness · blocker · launch window</div></div>
        <div class="board">
          ${reviewPosture()
            .map(
              (packet) => `<article class="pcard">
                  ...
                  <ul class="check">
                    ...
                    <li>${packet.launchWindowHours} hours to the next governance checkpoint</li>
...
export function renderVerification() {
  return layout(
    "Entra Guest Access Governor — Verification",
    "/verification",
    `<section class="section">
        ...
        <div class="stack">
          ${verification()
            .map(
              (item, index) => `<div class="src"><div class="src-name">verification ${index + 1}</div><div class="src-tit">${item}</div><p>This surface is built to stay honest about offline exports, synthetic sample data, and real guest-governance posture.</p></div>`
            )
            .join("")}
        </div>
      </section>`
...
export function renderDocs() {
  return layout(
    "Entra Guest Access Governor — Docs",
    "/docs",
    `<section class="section">
        <div class="sh"><h2>Docs</h2><div class="note">routes · cli · api</div></div>
        <div class="stack">
          <div class="src"><div class="src-name">routes</div><div class="src-tit">Public control surface</div><p><code>/</code>, <code>/guest-lane</code>, <code>/access-gaps</code>, <code>/review-posture</code>, <code>/verification</code>, <code>/docs</code></p></div>
          <div class="src"><div class="src-name">api</div><div class="src-tit">Structured payloads</div><p><code>/api/dashboard/summary</code>, <code>/api/guest-lane</code>, <code>/api/access-gaps</code>, <code>/api/review-posture</code>, <code>/api/verification</code>, <code>/api/sample</code></p></div>
          <div class="src"><div class="src-name">cli</div><div class="src-tit">Offline Entra guest analysis</div><p><code>npx entra-guest-access-governor fixtures/entra-guest-healthy.json --format summary</code> renders the same posture the dashboard exposes.</p></div>
        </div>
      </section>`
  );
}
